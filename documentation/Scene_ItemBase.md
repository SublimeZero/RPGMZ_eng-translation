...
Scene for using [Items] and [Skills].

Modified in v1.0.2.

Related Classes: [SceneManager](SceneManager.md), [Game_Action](Game_Action.md)<br />
Related Scenes: [Scene_Map](Scene_Map.md)
...
| --- | --- | --- |
| `_actorWindow` | [Window_MenuActor](Window_MenuActor.md) | Actor selection window |
| `_itemWindow` | [Window_Selectable](Window_Selectable.md) | Item and skill selection window |
...
**@MZ** Returns the rectangle area of the actor window.

#### applyItem ()
Applies the effect of the item.

#### canUse () → {Boolean}
Checks if the currently selected item can be used by the selected actor.

#### checkCommonEvent ()
Checks for [Common Events] and transitions to [Scene_Map](Scene_Map.md).

#### create ()
Override: [Scene_MenuBase](Scene_MenuBase.md#create-)

#### createActorWindow ()
Generates the actor window.

#### determineItem ()
Uses the currently selected item on the selected actor.

#### hideActorWindow ()
**@MZ** Hides the actor window.

#### initialize ()
Override: [Scene_MenuBase](Scene_MenuBase.md#initialize-)

#### isCursorLeft () → {Boolean}
Checks if the cursor is on the left.

#### isItemEffectsValid () → {Boolean}
Checks if the currently selected item has an effect on the selected actor.

#### item () → {[RPG.UsableItem](RPG.UsableItem.md)} 
Returns the item added to the item window.

#### itemTargetActors () → {[Game_Actor](Game_Actor.md)}
Returns the target actors.

#### onActorCancel ()
Handler called when the actor selection is canceled.

#### onActorOk ()
Handler called when the actor selection is confirmed.

#### showActorWindow ()
**@MZ** Displays the actor window.
